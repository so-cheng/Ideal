...
namespace Ideal.Ideal.WeChat
{
    ...
    public class OAuth2API
    {
        ...
        /// <returns></returns>
        public static string GetOpenIDAndToken(string code, string appid, string appSecret)
        {
            string resurnJosin = string.Empty;
            string text = "";
            string access_token = "";
            string openid = "";
            dynamic accessToken = GetAccessToken(code, appid, appSecret);
            dynamic val = OAuth2API.RefreshAccess_token(accessToken.refresh_token, appid);
            if (val != null)
            {
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public static dynamic GetOpenID(string appId, string secret, string code)
        {
            var client = new HttpClient();
            var result = client.GetAsync(string.Format("https://api.weixin.qq.com/sns/jscode2session?appid={0}&secret={1}&js_code={2}&grant_type=authorization_code", appId, secret, code)).Result;
